...
class _RecipeDetailScreenState extends ConsumerState<RecipeDetailScreen> {
    ...
    return Scaffold(
      ...
      body: SingleChildScrollView(
        ...
        child: Column(
          ...
          children: [
            // ── Hero card: category + description + cooking stats ──
            _card(
              child: Column(
                crossAxisAlignment: CrossAxisAlignment.start,
                children: [
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      if (recipe.hotHoldingRequired)
                        _pill('Hot Holding', const Color(0xFFDC2626), Icons.local_fire_department_rounded),
                      if (recipe.extraCareFlags.isNotEmpty)
                        ...recipe.extraCareFlags.map((f) =>
                          _pill(f, const Color(0xFFD97706), Icons.warning_amber_rounded)),
                    ],
                  ),
                  // Description
                  if (recipe.description != null && recipe.description!.isNotEmpty) ...[
                    ...
                    Text(recipe.description!,
                      style: GoogleFonts.inter(fontSize: 15, height: 1.5, color: AppColors.midText),
                    ),
                  ],
                  // Cooking stats row
                  if (hasCooking) ...[
                      ...
                      child: Row(
                        children: [
                          if (recipe.cookingTemp != null)
                            _statChip(Icons.thermostat_rounded, '${recipe.cookingTemp!.toStringAsFixed(0)}°C', const Color(0xFFEA580C)),
                          if (recipe.cookingTemp != null && recipe.cookingTime != null)
                            Container(width: 1, height: 28, margin: const EdgeInsets.symmetric(horizontal: 14), color: AppColors.divider),
                          if (recipe.cookingTime != null)
                            _statChip(Icons.timer_rounded, '${recipe.cookingTime!.toStringAsFixed(0)} ${recipe.cookingTimeUnit ?? 'min'}', const Color(0xFF2563EB)),
                          if ((recipe.cookingTemp != null || recipe.cookingTime != null) && recipe.cookingMethod != null)
                            Container(width: 1, height: 28, margin: const EdgeInsets.symmetric(horizontal: 14), color: AppColors.divider),
                          if (recipe.cookingMethod != null)
                            _statChip(Icons.outdoor_grill_rounded, recipe.cookingMethod!, const Color(0xFF059669)),
                        ],
                      ),
                    ),
                  ],
            ...
            if (hasAllergens) ...[
              ...
              _card(
                child: Column(
                  crossAxisAlignment: CrossAxisAlignment.start,
                  children: [
                    _sectionTitle('Allergens', Icons.warning_rounded, const Color(0xFFDC2626)),
                  ...
                  children: [
                    ...
                    ...recipe.ingredients!.asMap().entries.map((entry) {
                      ...
                      return Container(
                        ...
                        decoration: BoxDecoration(
                          color: entry.key.isEven ? const Color(0xFFF8FAFC) : Colors.transparent,
                          borderRadius: BorderRadius.circular(10),
                        ),
                        child: Row(
                          children: [
                            ...
                            Expanded(
                              child: Column(
                                ...
                                children: [
                                  Text(name,
                                    style: GoogleFonts.inter(fontSize: 15, fontWeight: FontWeight.w500, color: AppColors.darkText),
                                  ),
                                  if (hasQty)
                                    Text('${ri.quantity}${ri.unit != null ? ' ${ri.unit}' : ''}',
                ...
                child: Column(
                  ...
                  children: [
                    ...
                    Text(recipe.instructions,
                      style: GoogleFonts.inter(fontSize: 15, height: 1.7, color: AppColors.darkText),
                    ),
                  ],
                ),
              ),
              ...
              _card(
                child: Column(
                  ...
                  children: [
                    ...
                    const SizedBox(height: 14),
                    if (recipe.sfbbCheckMethod != null)
                      _safetyRow('Check Method', recipe.sfbbCheckMethod!),
                    if (recipe.reheatingInstructions != null)
                      _safetyRow('Reheating', recipe.reheatingInstructions!),
                    if (recipe.chillingMethod != null)
                      _safetyRow('Chilling', recipe.chillingMethod!),
                  ],
                ),
              ),
            ],
  ...
  Widget _sectionTitle(String title, IconData icon, Color color) {
    return Row(
      children: [
        ...
        Text(title,
          style: GoogleFonts.inter(fontSize: 16, fontWeight: FontWeight.w700, color: AppColors.darkText),
        ),
      ],
    );
  }
  ...
  Widget _safetyRow(String label, String value) {
    return Padding(
      ...
      child: Column(
        ...
        children: [
          Text(label,
            style: GoogleFonts.inter(fontSize: 12, fontWeight: FontWeight.w600, color: AppColors.lightText, letterSpacing: 0.3),
          ),
          const SizedBox(height: 4),
          Text(value,
            style: GoogleFonts.inter(fontSize: 14, height: 1.5, color: AppColors.darkText),
          ),
        ],
      ),
    );
