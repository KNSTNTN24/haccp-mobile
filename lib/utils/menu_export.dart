  ...
  pdf.addPage(
    pw.MultiPage(
      ...
      header: (context) => pw.Column(
        ...
        children: [
          pw.Text('Menu', style: pw.TextStyle(fontSize: 22, fontWeight: pw.FontWeight.bold)),
          pw.SizedBox(height: 4),
          pw.Text(businessName, style: pw.TextStyle(fontSize: 14, color: PdfColors.grey700)),
      ...
      footer: (context) => pw.Row(
        ...
        children: [
          pw.Text(businessName, style: pw.TextStyle(fontSize: 9, color: PdfColors.grey500)),
      ...
      build: (context) {
        ...
        for (final entry in sortedEntries) {
          ...
          final headers = includeAllergens ? ['Dish', 'Dietary', 'Allergens'] : ['Dish', 'Dietary'];

          final data = entry.value.map((r) {
            final row = [r.name, r.dietaryLabels.join(', ')];
            if (includeAllergens) {
              row.add(r.allAllergens.map((a) => a[0].toUpperCase() + a.substring(1)).join(', '));
            }
            return row;
          }).toList();
        ...
        widgets.add(pw.SizedBox(height: 8));
        widgets.add(pw.Text('Summary', style: pw.TextStyle(fontSize: 14, fontWeight: pw.FontWeight.bold)));
...
String generateMenuCsv({
  ...
  for (final cat in categoryOrder) {
    ...
    for (final r in catRecipes) {
      if (includeAllergens) {
        rows.add([
          cat.displayName, r.name, r.dietaryLabels.join(', '),
          r.allAllergens.map((a) => a[0].toUpperCase() + a.substring(1)).join(', '),
          r.description ?? '',
        ]);
      } else {
        rows.add([cat.displayName, r.name, r.dietaryLabels.join(', '), r.description ?? '']);
      }
    }
  }
